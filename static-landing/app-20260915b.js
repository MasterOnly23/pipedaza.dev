...
const visual = (project) => project.image
  ? `<div class="project-visual image-visual image-${project.imageShape || "desktop"}"><img src="${project.image}" alt="${project.imageAlt}" decoding="async">${projectMarker(project)}</div>`
  : `<div class="project-visual party-visual" aria-hidden="true">
      ...
      </div>${projectMarker(project)}
    </div>`;

const projectList = document.querySelector("#project-list");
projectList.innerHTML = projects.map((project, index) => `
  <article class="project-card project-${project.accent} ${index % 2 ? "project-reversed" : ""} reveal">
    ${project.url ? `<a href="${project.url}" target="_blank" rel="noreferrer" class="project-hitbox" aria-label="Abrir ${project.name} en una pestaña nueva"></a>` : ""}
    <div class="project-info">
      ...
      ${project.private ? `<p class="project-access">Proyecto privado · sin acceso público</p>` : ""}
    </div>${visual(project)}
  </article>`).join("");
